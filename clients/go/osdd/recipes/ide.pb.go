...
// Ide captures IDE-specific integrations exposed by a recipe.
type Ide struct {
...
type Ide_builder struct {
	...
	// Built-in commands that should appear inside the IDE palette.
	Commands *Commands
	// MCP (Model Context Protocol) servers to register with the IDE.
	Mcp *Mcp
	// Allow/deny policy. Those permissions are converted into IDE-specific instructions where possible.
...
// Mcp declares the set of MCP servers that should be launched.
type Mcp struct {
	state              protoimpl.MessageState `protogen:"opaque.v1"`
...
type Mcp_builder struct {
	...
	// Map of server identifier to its transport definition.
...
// McpServer describes how to connect to a remote or local MCP server.
...
type McpServer_builder struct {
	...
	// Transport information for the server.

	// Fields of oneof xxx_hidden_Type:
	// HTTP(S) endpoint implementing the MCP protocol.
	Http *HttpMcpServer
	// Process executed locally where stdin/stdout speak MCP.
...
// HttpMcpServer references an MCP server reachable over HTTP.
...
type HttpMcpServer_builder struct {
	...
	// Fully qualified URL of the server endpoint.
	Url string
}
...
// Command defines a command palette entry surfaced to the user.
type Command struct {
	state           protoimpl.MessageState `protogen:"opaque.v1"`
...
type Command_builder struct {
	...
	// Human-readable command name shown in the IDE.
	Name string
	// Source of the command implementation.
	From *CommandFrom
}
...
// CommandFrom lists supported sources for a command implementation. A command here is am AI-agent command, e.g. Claude's slash-command.
...
type CommandFrom_builder struct {
	...
	// Chosen source type containing the command definition.

	// Fields of oneof xxx_hidden_Type:
	// Fetch command from a Git repository.
	Github *osdd.GitReference
	// Execute a local command to fetch commands'content.
	Cmd *osdd.Exec
	// Inline command contents.
	Text *string
	// -- end of xxx_hidden_Type
}
...
// Details of how IDE should be executed.
type LaunchDetails struct {
	state          protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Cmd *osdd.Exec             `protobuf:"bytes,1,opt,name=cmd,proto3"`
	xxx_hidden_Dir string                 `protobuf:"bytes,2,opt,name=dir,proto3"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *LaunchDetails) Reset() {
	*x = LaunchDetails{}
	mi := &file_osdd_recipes_ide_proto_msgTypes[8]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *LaunchDetails) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*LaunchDetails) ProtoMessage() {}

func (x *LaunchDetails) ProtoReflect() protoreflect.Message {
	mi := &file_osdd_recipes_ide_proto_msgTypes[8]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
func (x *LaunchDetails) GetCmd() *osdd.Exec {
	if x != nil {
...
func (x *LaunchDetails) GetDir() string {
	if x != nil {
...
func (x *LaunchDetails) SetCmd(v *osdd.Exec) {
	x.xxx_hidden_Cmd = v
}

func (x *LaunchDetails) SetDir(v string) {
	x.xxx_hidden_Dir = v
}

func (x *LaunchDetails) HasCmd() bool {
	if x == nil {
		return false
	}
...
func (x *LaunchDetails) ClearCmd() {
	x.xxx_hidden_Cmd = nil
}

type LaunchDetails_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// Command to start an IDE.
	Cmd *osdd.Exec
	// Directory from which IDE should be started.
	Dir string
}

func (b0 LaunchDetails_builder) Build() *LaunchDetails {
	m0 := &LaunchDetails{}
	b, x := &b0, m0
	_, _ = b, x
...
const file_osdd_recipes_ide_proto_rawDesc = "" +
	...
	"\x04type\"F\n" +
	"\rLaunchDetails\x12#\n" +
	"\x03cmd\x18\x01 \x01(\v2\x11.osdd.common.ExecR\x03cmd\x12\x10\n" +
	"\x03dir\x18\x02 \x01(\tR\x03dirB5Z3github.com/opensdd/osdd-api/clients/go/osdd/recipesb\x06proto3"

var file_osdd_recipes_ide_proto_msgTypes = make([]protoimpl.MessageInfo, 10)
var file_osdd_recipes_ide_proto_goTypes = []any{
	...
	(*LaunchDetails)(nil),     // 8: osdd.recipes.ide.LaunchDetails
	nil,                       // 9: osdd.recipes.ide.Mcp.ServersEntry
	(*Permissions)(nil),       // 10: osdd.recipes.permissions.Permissions
...
var file_osdd_recipes_ide_proto_depIdxs = []int32{
	...
	10, // 2: osdd.recipes.ide.Ide.permissions:type_name -> osdd.recipes.permissions.Permissions
	9,  // 3: osdd.recipes.ide.Mcp.servers:type_name -> osdd.recipes.ide.Mcp.ServersEntry
	...
	11, // 8: osdd.recipes.ide.CommandFrom.github:type_name -> osdd.common.GitReference
	12, // 9: osdd.recipes.ide.CommandFrom.cmd:type_name -> osdd.common.Exec
	12, // 10: osdd.recipes.ide.LaunchDetails.cmd:type_name -> osdd.common.Exec
	2,  // 11: osdd.recipes.ide.Mcp.ServersEntry.value:type_name -> osdd.recipes.ide.McpServer
