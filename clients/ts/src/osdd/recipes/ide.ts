...
export interface McpServer {
  ...
  type?:
    ...
    { $case: "http"; value: HttpMcpServer }
    | //
    /** Process executed locally where stdin/stdout speak MCP. */
    { $case: "stdio"; value: StdioMcpServer }
    | undefined;
}
...
function createBaseLaunchDetails(): LaunchDetails {
  return { cmd: undefined, dir: "" };
}

export const LaunchDetails: MessageFns<LaunchDetails> = {
  encode(message: LaunchDetails, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.cmd !== undefined) {
      Exec.encode(message.cmd, writer.uint32(10).fork()).join();
    }
    if (message.dir !== "") {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): LaunchDetails {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    const end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseLaunchDetails();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1: {
          ...
          message.cmd = Exec.decode(reader, reader.uint32());
          continue;
        }
        case 2: {
          if (tag !== 18) {
  ...
  fromJSON(object: any): LaunchDetails {
    return {
      cmd: isSet(object.cmd) ? Exec.fromJSON(object.cmd) : undefined,
      dir: isSet(object.dir) ? gt.String(object.dir) : "",
    };
  },

  toJSON(message: LaunchDetails): unknown {
    const obj: any = {};
    if (message.cmd !== undefined) {
      obj.cmd = Exec.toJSON(message.cmd);
    }
    if (message.dir !== "") {
      obj.dir = message.dir;
    }
  ...
  create(base?: DeepPartial<LaunchDetails>): LaunchDetails {
    return LaunchDetails.fromPartial(base ?? {});
  },
  fromPartial(object: DeepPartial<LaunchDetails>): LaunchDetails {
    const message = createBaseLaunchDetails();
    message.cmd = (object.cmd !== undefined && object.cmd !== null) ? Exec.fromPartial(object.cmd) : undefined;
